...
const TYPE_STYLES: Record<ThreadTweet['type'], { label: string; className: string }> = {
  hook:    { label: 'Hook',    className: 'bg-orange-50 text-orange-700 border border-orange-200/60' },
  content: { label: 'Content', className: 'bg-sky-50 text-sky-700 border border-sky-200/60' },
  bridge:  { label: 'Bridge',  className: 'bg-amber-50 text-amber-700 border border-amber-200/60' },
  cta:     { label: 'CTA',     className: 'bg-emerald-50 text-emerald-700 border border-emerald-200/60' },
}
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex flex-col items-center">
        ...
        {tweet.number < total && (
          <div className="mt-1 w-0.5 flex-1 bg-gradient-to-b from-orange-200 to-transparent" />
        )}
      </div>
      ...
      <div className="flex-1 pb-4">
        <div className="mb-1.5 flex items-center gap-2">
          <span className={cn('rounded-full px-2.5 py-0.5 text-[11px] font-medium', typeStyle.className)}>
            {typeStyle.label}
          </span>
    ...
    <div className="space-y-4">
      ...
      {tweets.length > 0 && !isGenerating && (
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => onSave(thread)} className="border-border/60">
            <svg className="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
