          ...
          {[
            {
              ...
              color: '#0EA5E9',
              spark: [35, 42, 38, 52, 48, 61, 58, 72, 68, 85, 78, 100],
            },
            {
              ...
              color: '#EA580C',
              spark: [45, 52, 48, 58, 54, 62, 58, 68, 72, 75, 80, 85],
            },
            {
              label: 'Posts Published',
              value: kpi.posts,
              delta: kpi.postsDelta,
              suffix: ' vs prior',
              icon: (
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
              ...
              color: '#8B5CF6',
              spark: [20, 25, 30, 28, 35, 40, 45, 50, 55, 60, 70, 75],
            },
            {
              label: 'Followers Gained',
              value: `+${kpi.followers}`,
              delta: kpi.followersDelta,
              icon: (
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
              ...
              color: '#10B981',
              spark: [30, 35, 32, 40, 38, 48, 52, 58, 65, 72, 80, 90],
            },
          ].map((stat) => (
            <div
              key={stat.label}
              className="group relative rounded-2xl border border-border/60 bg-card p-5 shadow-sm hover:shadow-md hover:border-border transition-all overflow-hidden"
            >
              {/* Background sparkline */}
              <div className="absolute inset-x-0 bottom-0 h-16 opacity-[0.08] group-hover:opacity-[0.12] transition-opacity">
                <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="h-full w-full">
                  <path
                    d={`M0,${40 - (stat.spark[0] / 100) * 40} ${stat.spark.map((v, i) => `L${(i / (stat.spark.length - 1)) * 100},${40 - (v / 100) * 40}`).join(' ')} L100,40 L0,40 Z`}
                    fill={stat.color}
                  />
                </svg>
              </div>
