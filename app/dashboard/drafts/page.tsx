...
export default function DraftsPage() {
  ...
  const handleCopy = useCallback(async (draft: { content: string; hashtags: string[] | null }) => {
    const fullContent = draft.hashtags && draft.hashtags.length > 0
      ? `${draft.content}\n\n${draft.hashtags.map(t => `#${t}`).join(' ')}`
      : draft.content

    await navigator.clipboard.writeText(fullContent)
    toast.success('Copied to clipboard!')
    setCopiedId(draft.content)
  ...
  const getToneName = (toneId: string | null) => {
    if (!toneId) return 'Default'
  ...
  if (!authLoading && !user) {
    return (
      <div className="flex flex-col">
        <Header title="Drafts" description="Your saved content ready to post" />
        <div className="p-6">
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-lg font-semibold">Sign in to save drafts</h3>
              <p className="mt-2 max-w-sm text-muted-foreground">
                Create an account to save your generated content and access it from anywhere.
              </p>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {draft.platforms.map((p) => (
                        <div key={p} className="flex h-6 w-6 items-center justify-center rounded-full bg-muted">
                          <PlatformIcon platform={p as PlatformId} className="h-3.5 w-3.5" />
                        </div>
                      ))}
                    </div>
                    <Badge variant="secondary" className="text-xs">
                      {getToneName(draft.tone as ToneId)}
                    </Badge>
                  </div>
                </CardHeader>
