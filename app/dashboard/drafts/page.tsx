  ...
  return (
    <div className="flex min-h-full flex-col">
      <Header
        ...
        action={
          <Button asChild className="btn-gradient h-10 rounded-full px-5 text-sm font-semibold">
      ...
      <div className="px-6 pt-7 pb-14 md:px-10 space-y-6">
        {/* Custom tab bar */}
        <div className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-card p-1 shadow-sm">
          {TABS.map((tab) => {
            ...
            const label = tab === 'posts' ? 'Posts' : 'Threads'
            return (
              <button
                key={tab}
                type="button"
                ...
                className={cn(
                  'inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold transition-all duration-200',
                  activeTab === tab
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    activeTab === tab ? 'bg-white/25 text-white' : 'bg-muted text-muted-foreground',
                  )}
                >
                  {count}
                </span>
          ...
          drafts.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {drafts.map((draft) => (
                <Card
                  key={draft.id}
                  className="flex flex-col overflow-hidden border-border/60 transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-[0_18px_40px_-20px_oklch(0.652_0.214_36_/_0.25)]"
                >
                  ...
                  <CardContent className="flex flex-1 flex-col">
                    <p className="flex-1 text-sm leading-relaxed line-clamp-4 text-foreground/90">{draft.content}</p>
                    ...
                    {draft.hashtags.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1">
                        {draft.hashtags.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full border border-primary/20 bg-primary/[0.08] px-2 py-0.5 text-[11px] font-semibold text-primary"
                          >
                            #{tag}
                          </span>
                        ))}
                    ...
                    <div className="mt-4 flex gap-2">
                      ...
                      >
                        {copiedId === draft.id ? (
                          <>
                            <svg className="mr-1 h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor">
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="font-display text-base leading-tight tracking-tight line-clamp-2">
                        {thread.title}
                      </h3>
                      <span className="shrink-0 rounded-full border border-primary/20 bg-primary/[0.08] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] text-primary">
                  ...
                  <CardContent className="flex flex-1 flex-col">
                    <div className="relative flex-1 rounded-xl border border-border/60 bg-muted/30 p-4">
                      <span
                        aria-hidden
                        className="absolute left-4 top-4 flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-primary to-accent text-[10px] font-bold leading-none text-white"
                      >
                        1
                      </span>
                      <div className="ml-8">
                        <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
                          {thread.tweets[0]?.type ?? 'Hook'}
                        </p>
  ...
  return (
    <Card className="relative overflow-hidden border-dashed border-border/60">
      ...
      <CardContent className="relative flex flex-col items-center justify-center py-20 text-center">
        <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 to-accent/20 ring-1 ring-primary/20">
          <svg
        ...
        <h3 className="font-display text-2xl leading-tight tracking-tight text-foreground">{title}</h3>
        <p className="mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">{description}</p>
        <Button asChild className="btn-gradient mt-6 h-10 rounded-full px-6 text-sm font-semibold">
          <Link href="/dashboard/create">
            <svg className="mr-1.5 h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
            </svg>
